...
const mockCommit1 = {
  sha: 'sha1',
  commit: { author: { name: 'author1', date: '2025-11-13T12:00:00Z' }, message: 'feat: commit 1' },
};
const mockCommit2 = {
  sha: 'sha2',
  commit: { author: { name: 'author2', date: '2025-11-13T10:00:00Z' }, message: 'fix: commit 2' },
};
const mockCommit3 = {
  sha: 'sha3',
  commit: { author: { name: 'author1', date: '2025-11-13T14:00:00Z' }, message: 'docs: commit 3' },
};
const mockCommitMain = {
  sha: 'sha_main',
  commit: {
    author: { name: 'author_main', date: '2025-11-13T11:00:00Z' },
    message: 'merge commit',
  },
};
const mockCommitDev = {
  sha: 'sha_dev',
  commit: { author: { name: 'author_dev', date: '2025-11-13T13:00:00Z' }, message: 'dev commit' },
};
...
describe('commitService', () => {
  ...
  describe('getCommits (all branches)', () => {
    it('should fetch from all branches, deduplicate, and sort commits', async () => {
      ...
      expect(mockedFetchFromGitHub).toHaveBeenCalledWith(
        'https://api.github.com/repos/owner/repo/commits?sha=develop&since=since&until=until',
        'owner',
        'repo'
      );
